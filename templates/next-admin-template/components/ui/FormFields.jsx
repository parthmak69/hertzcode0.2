...
export function Toggle({ label, description, className = '', checked = false, disabled = false, ...props }) {
    return (
        <label className={`flex items-start gap-3 cursor-pointer text-foreground ${className}`}>
            <span className={`relative inline-flex h-6 w-11 flex-shrink-0 items-center rounded-full transition-colors duration-200 ${checked ? 'bg-primary' : 'bg-secondary'} ${disabled ? 'opacity-60 cursor-not-allowed' : ''}`}>
                <input
                    type="checkbox"
                    checked={checked}
                    disabled={disabled}
